...
A standard Next.js layout: `package.json` and `src/` are at the repository root,
so Vercel and any other build tool detect the framework with no configuration.
...
`supabase/migrations/0008_default_workflow.sql`.

## Tests

```bash
# needs a local Postgres; see the header of each file
psql -f supabase/testing/00_auth_shim.sql   # stands in for Supabase's auth schema
psql -f supabase/testing/01_smoke_test.sql  # handoff engine, 10 cases
psql -f supabase/testing/02_workflow_test.sql  # full 11-stage walk, both PO branches
```

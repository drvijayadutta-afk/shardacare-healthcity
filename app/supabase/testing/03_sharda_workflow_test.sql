...
BEGIN
  ...
  RAISE NOTICE '%', rtrim(v_path,' > ');
  IF v_path NOT LIKE '%FINAL_APPROVAL > PO_REQUEST > PROCUREMENT_REVIEW > PO_APPROVAL > PO_RELEASED > PRODUCTION%'
    THEN RAISE EXCEPTION 'PO detour wrong: %', v_path; END IF;
  RAISE NOTICE 'PASS  PO path tracked through procurement';
END
$t2$;
